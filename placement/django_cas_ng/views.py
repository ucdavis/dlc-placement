...
def login(request, next_page=None, required=False):
    ...
    if ticket:
        ...
        if user is not None:
            ...
            SessionTicket.objects.create(  # @UndefinedVariable
            ...
            if pgtiou and settings.CAS_PROXY_CALLBACK:
                # Delete old PGT
                ProxyGrantingTicket.objects.filter(  # @UndefinedVariable
                    user=user,
                    session_key=request.session.session_key
                ).delete()  # @UndefinedVariable
                # Set new PGT ticket
                try:
                    pgt = ProxyGrantingTicket.objects.get(pgtiou=pgtiou)  # @UndefinedVariable
                    pgt.user = user
...
def logout(request, next_page=None):
    ...
    try:
        st = SessionTicket.objects.get(session_key=request.session.session_key)  # @UndefinedVariable
        ticket = st.ticket
    ...
    ProxyGrantingTicket.objects.filter(session_key=request.session.session_key).delete()  # @UndefinedVariable
    SessionTicket.objects.filter(session_key=request.session.session_key).delete()  # @UndefinedVariable
...
def callback(request):
    ...
    if request.method == 'POST' and request.POST.get('logoutRequest'):
        ...
        pgt = ProxyGrantingTicket.objects.create(pgtiou=pgtiou, pgt=pgtid)  # @UndefinedVariable
        pgt.save()
        ProxyGrantingTicket.objects.filter(  # @UndefinedVariable
            session_key=None,
...
def clean_sessions(client, request):
    for slo in client.get_saml_slos(request.POST.get('logoutRequest')):
        try:
            st = SessionTicket.objects.get(ticket=slo.text)  # @UndefinedVariable
            ...
            ProxyGrantingTicket.objects.filter(session_key=st.session_key).delete()  # @UndefinedVariable
            SessionTicket.objects.filter(session_key=st.session_key).delete()  # @UndefinedVariable
        except SessionTicket.DoesNotExist:
            pass
